...
## 1. ✅ Поддержка imagePullSecrets (РЕАЛИЗОВАНО)

> **Статус:** Реализовано в k8app v3.4.0+

k8app теперь поддерживает универсальный `imagePullSecrets`:
...
Legacy флаги (`deploySecretHarbor`, `deploySecretNexus`) продолжают работать для обратной совместимости.

### Использование

1. Создать secret в namespace:
   ```bash
   ./scripts/setup-registry-secret.sh
   ```

2. Добавить в values сервиса:
   ```yaml
   # .cicd/default.yaml
   imagePullSecrets:
     - name: regsecret
